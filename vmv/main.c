...
int		steps_estimation(t_operation	operation)
{
	int s;
	int i;
	int tl;
	...
	s = 1;
	if (operation.code != 1 && operation.code != 9 && operation.code != 12 & operation.code != 15)
		s += 1;
	i = -1;
	while (++i < 3)
	{
		tl = 1;
		if (operation.arg_types[i] == 2){
			if (operation.code < 8 || operation.code == 13 || operation.code == 16)
				tl = 4;
		} else if (operation.arg_types[i] == 3)
			tl = 2;
		s += tl;
...
void	exec_function(t_list *lst)
{
	...
	if (operation)
	{
		...
		carriage->pos = (carriage->pos + steps_estimation(tmp)) % MEM_SIZE;// todo length estimation
	}
	else
		carriage->pos++;
}
